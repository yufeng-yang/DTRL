"""Evaluate the five Table 2 methods on Semicircle-Wide.

Environment ``SafetyPointSemicircle0-v6``. Loads trained weights only.
Pins this process to ``CPU_CORE`` (default 0).

Classes: SafetyToGymnasiumWrapper, PpoActor, SacActor, OffExit1/2/3,
TanhWrap, ExitActor, LatencyBuffer, EpisodeStats.
Functions: _setup_cpu, _make_env, _goal_achieved, _build_exit,
_build_off_exits, _to_env_action, _timed_exit_infer, _run_env_steps,
_warmup_exit_into_buffer, _budget_tags, _episode_success, _select_exit,
_run_episode, _run_fixed_policy_episode, _summarize, _print_progress,
_eval_dynamic, _eval_fixed, main.
"""
...
_ROOT = _CODE / "Src_Training" / "Semicircle-Wide"
...
SUCCESS_RETURN_THRESH = 9.0  # success requires goal-at-end and return above this

# The DTRL-On exits define the common runtime budget used by all methods.

SWI_ZIP = _ROOT / "SWI/runs/SWI_20260517_130543/checkpoints/SWI_600000_steps.zip"
FULL_ZIP = _ROOT / "Full/runs/Full_20260517_125954/best_model/Full_best_model.zip"
MMS_E2_ZIP = _ROOT / "MMS/runs/MMS_20260517_141104/best_model/MMS_best_model.zip"
JOINT_MODEL = (
    _ROOT
    / "DTRL-Off/runs/DTRL-Off_20260517_205942/best_model/DTRL-Off_best_model.pt"
)
...
_EXITS: list[tuple[str, Path, str, int]] = [
    (
        "e1",
        _ROOT
        / "DTRL-On/runs/DTRL-On_e1_20260517_161241/best_model/DTRL-On_e1_best_model.zip",
        "ppo",
        1,
    ),
    (
        "e2",
        _ROOT
        / "DTRL-On/runs/DTRL-On_e2_20260517_190831/best_model/DTRL-On_e2_best_model.zip",
        "ppo",
        2,
    ),
    (
        "ef",
        _ROOT
        / "DTRL-On/runs/DTRL-On_full_20260517_145457/best_model/DTRL-On_full_best_model.zip",
        "ppo",
        3,
    ),
]
...
def _episode_success(ep_ret: float, goal_at_end: bool) -> bool:
    return bool(goal_at_end and ep_ret > SUCCESS_RETURN_THRESH)
...
def _run_episode(
    ...
    return EpisodeStats(
        ...
        n_steps=step,
        deadline_hits=hits,
        success=_episode_success(ep_ret, goal),
    )
...
def main() -> None:
    ...
    print(f"  success = goal_achieved and no crash")
    for name, results in (
        ("FULL", full_results),
        ("SWI", swi_results),
        ("MMS", mms_results),
