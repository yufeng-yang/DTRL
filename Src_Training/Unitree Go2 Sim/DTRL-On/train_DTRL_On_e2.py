"""Go2 PPO Stage-E2: Freeze full obs→512→256→128, and only retrain 128→action.

Load actor ``mlp.0`` / ``mlp.2`` / ``mlp.4`` from full checkpoint (consistent with the first three layers of full ``hidden_dims``) and freeze;
This script actor ``hidden_dims=[512, 256, 128]`` only trains ``mlp.6`` (128→12) and ``distribution``.

Default pretrained weights::

    DTRL-On/runs/onpolicy/DTRL-On_full_20260516_092741_449462/DTRL-On_full_8000.pt

Training: 3000 iter; ``save_interval=300``; ``log_interval=300``.

Run::

    python unitree_go2/DTRL-On/ppo_e2_train.py"""
...
# Linear layer (Sequential subscript) in the E2 actor that is aligned with full and needs to be loaded and frozen
_E2_FROZEN_ACTOR_LINEAR_KEYS = ("mlp.0", "mlp.2", "mlp.4")
...
def _resolve_locomotion_dir(explicit: str | None = None) -> Path:
    ...
    raise FileNotFoundError(
        ...
        + "\nYou can set GENESIS_LOCOMOTION / RTSS_ROOT, or use --genesis-locomotion."
    )
...
def _load_linear_from_full(
    actor_mlp: torch.nn.Sequential,
    full_sd: dict,
    mlp_key: str,
) -> None:
    """Copy one layer of Linear (mlp_key such as ``mlp.0``) from full state_dict and freeze it."""
    layer_idx = int(mlp_key.split(".")[1])
    layer = actor_mlp[layer_idx]
    if not isinstance(layer, torch.nn.Linear):
        raise TypeError(f"Expected Linear, Actual {mlp_key} -> {type(layer)}")
    ...
    for name in expected:
        ...
        if tuple(full_sd[key].shape) != tuple(expected[name].shape):
            raise ValueError(
                f"{key} Shape mismatch: full {tuple(full_sd[key].shape)} "
                f"vs E2 {tuple(expected[name].shape)}"
            )
...
def load_and_freeze_actor_trunk_e2(runner: OnPolicyRunner, pretrain_path: Path) -> None:
    """Freeze full's obs→512→256→128 (actor mlp.0 / mlp.2 / mlp.4)."""
    ...
    for mlp_key in _E2_FROZEN_ACTOR_LINEAR_KEYS:
        _load_linear_from_full(actor_mlp, full_sd, mlp_key)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Go2 PPO E2: freeze full obs→512→256→128, retrain 128→out")
    ...
    p.add_argument(
        ...
        help="full PPO model_<iter>.pt, load and freeze actor mlp.0/2/4",
    )
...
def main() -> None:
    ...
    print(f"[E2] Genesis locomotion: {locomotion_dir}")
    ...
    run_id = f"ppo_e2_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
    ...
    meta = {
        ...
        "actor_arch": "obs-512-256-128-out (frozen mlp.0/2/4)",
        "frozen_actor_keys": list(_E2_FROZEN_ACTOR_LINEAR_KEYS),
        "critic_arch": "[512, 256, 128] (trainable)",
    }

    n_pe = train_cfg["num_steps_per_env"]
    print(
        f"[E2] PPO iter={args.max_iterations} | num_steps_per_env={n_pe} | num_envs={args.num_envs} | "
        f"save/log every {args.save_interval} iter"
    )
    print(f"[E2] Pre-training (freeze mlp.0/2/4): {pretrain_path}")
    print(f"[E2] Output: {log_dir}")
    ...
    load_and_freeze_actor_trunk_e2(runner, pretrain_path)

    trainable = _trainable_actor_param_names(runner.alg.actor)
    print(f"[E2] actor trainable parameters ({len(trainable)}):")
    for n in trainable:
        print(f"  - {n}")
