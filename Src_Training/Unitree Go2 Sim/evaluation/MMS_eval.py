"""Go2 MMS dynamic port selection formal evaluation (SWI e1 / MMS-2 e2 / full ef).

e1: b2_owf SWI (obs→512→out)
e2: b3_mms MMS-2 (obs→512→256→128→out)
ef: b1_full (obs→512→256→128→128→128→out)

Select the deepest feasible exit according to the deadline and p97 of each port's latency buffer; miss → reuse the previous action.
Task go2_walk_easy, each round vx∈[-1,1] is uniformly random, vy=0, yaw=0.
PPO mean direct step (not multiplied by action_range).

Warmup 1000 steps + fill buffer for each port 1000 steps + formal evaluation.
E_x still starts 50 steps; deadline hit rate counts all steps in the entire process (including the start).
success = running full episode and E_x (vx only) < 0.1.

Run::

    python unitree_go2/evaluation/mms_eval.py
    python unitree_go2/evaluation/mms_eval.py --n-episodes 10"""
...
from time_calculation import load_onpolicy_ppo  # noqa: E402

DEFAULT_MODELS: dict[str, Path] = {
    "e1": _ROOT / "SWI/runs/SWI_20260516_090620_581385/SWI_999.pt",
    "e2": _ROOT / "MMS/runs/MMS_20260516_105335_797134/MMS_3500.pt",
    "ef": _ROOT / "Full/full_policy/Full_20260516_092741_449462/Full_8000.pt",
}

_LOADER_TAG = {"e1": "e1", "e2": "e2", "ef": "efull"}

ExitName = Literal["e1", "e2", "ef"]
EXIT_TAGS: tuple[ExitName, ...] = ("e1", "e2", "ef")
...
def _load_mms_exits(
    ...
    for tag in EXIT_TAGS:
        nets[tag] = load_onpolicy_ppo(model_paths[tag], _LOADER_TAG[tag], device)  # type: ignore[arg-type]
    return nets
...
def _resolve_model_paths(args: argparse.Namespace) -> dict[ExitName, Path]:
    return {
        ...
        "ef": args.model_ef.expanduser().resolve(),
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Go2 MMS dynamic port selection formal evaluation (e1/e2/ef)")
    ...
    parser.add_argument(
        "--model-e1",
        type=Path,
        default=DEFAULT_MODELS["e1"],
        help="SWI (e1) checkpoint",
    )
    parser.add_argument(
        "--model-e2",
        type=Path,
        default=DEFAULT_MODELS["e2"],
        help="MMS-2 (e2) checkpoint",
    )
    parser.add_argument(
        "--model-ef",
        type=Path,
        default=DEFAULT_MODELS["ef"],
        help="full (ef) checkpoint",
    )
    ...
    _setup_cpu()
    nets = _load_mms_exits(model_paths, device)

    print("Weight: MMS Three Ports PPO")
    print(f"  [e1] SWI  {model_paths['e1']}")
    print(f"  [e2] MMS2 {model_paths['e2']}")
    print(f"  [ef] full {model_paths['ef']}")
    print(
    ...
    print(f"\n{'=' * 70}")
    print(f"comprehensive ({n_episodes} episodes) — Go2 MMS:")
