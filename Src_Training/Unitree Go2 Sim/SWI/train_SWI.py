...
_LOCOMOTION = Path(__file__).resolve().parents[2] / "Genesis" / "examples" / "locomotion"
if _LOCOMOTION.is_dir():
    sys.path.insert(0, str(_LOCOMOTION))
else:
    raise FileNotFoundError(
        f"not found {_LOCOMOTION}, please confirm that the RTSS/Genesis submodule exists."
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Only the run_name of train_cfg is written; the saving directory name is fixed to swi_<timestamp>, which is the same as the directory name by default",
    )
    ...
    # Separate directory for each run: b2_owf/runs/swi_YYYYMMDD_HHMMSS_ffffff/
    _RUNS_ROOT.mkdir(parents=True, exist_ok=True)
    run_id = f"swi_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
...
"""# Network: obs -> 512 -> out; instructions vx∈[-1,1] vy∈[-0.5,0.5] yaw=0
#Default 1000 iter, save_interval=100 → model_0,100,...,1000
# Output: unitree_go2/SWI/runs/swi_<timestamp>/
# python unitree_go2/SWI/train_swi.py"""
