"""Semicircle v5 PPO Stage-E2: Freeze the obs→256→128→128 of full PPO, and only retrain the action header.

Loading the best weights from ``train_ppo_full`` (``pi=[256,128,128,64,64]``)
``policy_net[0,2,4]``(obs→256→128→128) and freeze; this script ``pi=[256,128,128]``, only train
``action_net`` (128→out) and independent Critic ``vf=[256,128,128]``.

Run: conda activate sb3sg && python dtrl_e2.py"""
...
# Aligned with train_ppo_full output; can be changed to your absolute path at the end of main()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="PPO E2 stage on Semicircle v5 (frozen 256-128-128 trunk).")
