"""Training SAC (Stable-Baselines3) on Semicircle **v5**.

Default environment ``SafetyPointSemicircle0-v5``; logs and checkpoints are written by default
``<this script directory>/runs/full_sac_<timestamp>/`` (i.e. ``Full/runs/...``).
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train SAC on Safety*Semicircle0-v5 (default Point) with SB3."
    )
    parser.add_argument(
        "--env-id",
        type=str,
    ...
    parser.add_argument("--eval-freq", type=int, default=20_000, help="Best-model eval frequency in env steps.")
...
def main():
    ...
    policy_kwargs = dict(
        net_arch=dict(
            # obs → 256 → 128 → 128 → 64 → 64 → output (the last layer of SB3 is connected by the frame to the action/scalar Q)
