"""Semicircle v5 three-port joint SAC (large: e1 deepened to 256→128).
...
Run: conda activate sb3sg && python large_joint_train.py"""
...
from eenn_network import ActorEENN3SemicircleLarge, CriticDeepTwin, soft_update
...
def assign_triple_joint_grads_large(
    ...
    """e1 updates backbone+fc2+e1; e2 updates to fc3; efull updates the full depth segment."""
...
def run_exit_episode(
    env_id: str,
    actor: ActorEENN3SemicircleLarge,
    device: torch.device,
...
def train(args: argparse.Namespace) -> None:
    ...
    actor = ActorEENN3SemicircleLarge(obs_dim, act_dim).to(device)
    ...
    run_dir = os.path.join(args.runs_root, f"large_joint_sac_{timestamp}")
    ...
    while global_step < args.total_timesteps:
        ...
        if do_train:
            ...
            if in_warmup:
                ...
                assign_triple_joint_grads_large(
        ...
        if global_step >= next_save_at:
            while global_step >= next_save_at:
                next_save_at += args.save_freq
            ckpt_path = os.path.join(ckpt_dir, f"large_joint_sac_{global_step}_steps.pt")
            save_checkpoint(
                ckpt_path,
                step=global_step,
                actor=actor,
    ...
    final_path = os.path.join(final_model_dir, "large_joint_sac_final.pt")
    save_checkpoint(
        final_path,
        step=global_step,
        actor=actor,
