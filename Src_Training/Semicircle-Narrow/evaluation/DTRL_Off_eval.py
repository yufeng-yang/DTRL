"""Semicircle narrow (v5) EENN dynamic deadline + select the deepest exit according to delay buffer (off-policy DTRL).

Environment: SafetyPointSemicircle0-v5
Every 10 steps: deadline∈[0.08, 0.15] ms uniform sampling, select ports according to the p97 quantile of each port buffer.
miss deadline → reuse the previous action. Each port first warms up for 1000 steps (without writing the buffer), then runs 1000 steps to fill the buffer, and then formally evaluates 100 episodes (single core).

Run: python dtrl_offpolicy_evaluation.py"""
...
def main() -> None:
    ...
    print(f"\n{'=' * 70}")
    print(f"comprehensive ({N_SEEDS} seeds) — Semicircle narrow off-policy DTRL:")
