"""Semicircle narrow (v5) Fixed OWF shallow PPO formal evaluation (same time constraints as full_eval).

Strategy: b2_owf PPO obs→256→out
Every 10 steps: deadline∈[0.08, 0.15] ms; miss → reuse the previous step action.
Forward: Gaussian mean + clip (not predict). Real environment warmup for 1000 steps.
...
MODEL_ZIP = Path(
"/home/yufeng.yang/codespace/Prepare_to_transfer/checkpoints/Semicircle-Narrow/SWI/weights/SWI_1500000_steps.zip"
)
...
class PpoOwfActor(nn.Module):
    """OWF shallow PPO: policy_net → action_net (Gaussian mean, clip during inference)."""
...
def _build_actor(model_zip: Path) -> tuple[PpoOwfActor, str]:
...
def _timed_infer(
    actor: PpoOwfActor, obs: np.ndarray, action_space: spaces.Box
) -> tuple[np.ndarray, float]:
    t0 = time.perf_counter()
    with torch.inference_mode():
        x = torch.as_tensor(obs, dtype=torch.float32).unsqueeze(0)
        mean = actor(x).detach().cpu().numpy().reshape(-1)
...
def _warmup(actor: PpoOwfActor, env_id: str, action_space: spaces.Box, seed: int) -> None:
...
def _run_episode(
    actor: PpoOwfActor, env_id: str, action_space: spaces.Box, seed: int
) -> EpisodeStats:
    _setup_cpu(quiet=True)
    env = _make_env(env_id)
...
def main() -> None:
    ...
    print("Strategy: OWF shallow PPO (b2_owf, obs→256→out)")
    ...
    print(f"\n{'=' * 70}")
    print(f"comprehensive ({N_SEEDS} seeds) — OWF shallow PPO:")
