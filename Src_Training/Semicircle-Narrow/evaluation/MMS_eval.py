"""Semicircle narrow (v5) MMS dynamic port selection evaluation (same rules as dtrl_offpolicy_evaluation).

e1: OWF PPO (obs→256) | e2: MMS M2 PPO (obs→256→128→128) | ef: full SAC (deep MLP)
...
def main() -> None:
    ...
    arch = {
        "e1": "OWF PPO obs→256→out (Gaussian mean)",
        "e2": "MMS M2 PPO obs→256→128→128→out (Gaussian mean)",
        "ef": "full SAC obs→256→128→128→64→64→out (tanh+unscale)",
    }

    exits: dict[str, ExitActor] = {}
    buffers: dict[str, LatencyBuffer] = {}
    print("Loading three-port MMS:")
    ...
    print(f"\n{'=' * 70}")
    print(f"comprehensive ({N_SEEDS} seeds) — Semicircle narrow MMS dynamic port selection:")
