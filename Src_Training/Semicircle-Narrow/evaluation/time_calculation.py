"""Semicircle narrow: Separate the e1/e2/e3 independent model from the joint SAC, and then measure the return and inference time.

1. Export: evaluation/standalone_models/DTRL-Off_20260516_034629/e1.pt e2.pt e3.pt
2. Test: warm-up 1 episode, official 10 episodes (seeds 42–51)
...
STANDALONE_DIR = _EVAL / "standalone_models" / "DTRL-Off_20260516_034629"
START_SEED = 42
N_EPISODES = 10
CPU_CORE = 2
...
def _dist_stats(arr: np.ndarray) -> dict[str, float]:
    d = {"mean": float(arr.mean()), "median": float(np.median(arr)), "std": float(arr.std(ddof=0))}
    for p in _PCTS:
        d[f"p{p}"] = float(np.percentile(arr, p))
    return d


def _print_latency(title: str, dist: dict[str, float], extra: str = "") -> None:
...
def _print_return(title: str, returns: list[float]) -> None:
    arr = np.asarray(returns, dtype=np.float64)
    d = _dist_stats(arr)
    per_ep = ", ".join(f"{r:.4f}" for r in returns)
    print(f"\n=== {title} ===")
    print(
        f"  mean={d['mean']:.4f}  std={d['std']:.4f}  median={d['median']:.4f}  "
        f"min={float(arr.min()):.4f}  max={float(arr.max()):.4f}"
    )
    print(f"  per-ep: {per_ep}")
...
def main() -> None:
    ...
    env_id = joint_ckpt.get("env_id", "SafetyPointSemicircle0-v5")
    ...
    print("Step 1/2 — Manually unpack and export e1/e2/e3 from joint weights:")
    ...
    print(
        f"\nStep 2/2 — Load and evaluate from standalone .pt | env_id={env_id}\n"
        f"Preheat=1 episode (seed={START_SEED}) | formal ={N_EPISODES} episodes (seeds {seeds}) | "
        f"Reasoning=tanh(mean)+unscale"
    )
    ...
    for tag, _, arch in _EXITS:
        ...
        _print_return(f"{tag} return ({N_EPISODES} episodes)", returns)
        _print_latency(
            f"{tag} Reasoning takes time ({len(eval_times)} steps)",
            _dist_stats(eval_times),
        )
