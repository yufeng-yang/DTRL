"""Training PPO on Semicircle **v5** (MMS M2, Stable-Baselines3).
...
``<this script directory>/runs/mms_m2_ppo_<timestamp>/`` (i.e. ``b3_mms/runs/...``).
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train PPO (MMS M2) on Safety*Semicircle0-v5 (default Point) with SB3."
    )
    parser.add_argument(
        "--env-id",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Run the root directory; mms_m2_ppo_<timestamp>/(checkpoints, tb, final_model) will be created under it."
...
def main():
    ...
    policy_kwargs = dict(
        net_arch=dict(
            # obs → 256 → 128 → 128 → Output (the last layer of SB3 is connected by the frame to the action distribution/scalar V)
            pi=[256, 128, 128],
            vf=[256, 128, 128],
        ),
    ...
    model.learn(
        ...
        tb_log_name="ppo_mms_m2",
        progress_bar=args.progress_bar,
    )

    final_model_path = os.path.join(final_model_dir, "ppo_mms_m2_final")
