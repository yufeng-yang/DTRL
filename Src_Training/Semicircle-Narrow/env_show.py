"""Semicircle v6 environment preview: Load the track and drive manually with the keyboard.

Execute in the ``Semicircle-Narrow`` directory::
...
Environment variables (must take effect before import mujoco, set by this script before import):
- ``SEMICIRCLE_MUJOCO_GL`` / ``MUJOCO_GL``: MuJoCo backend
...
def run_teleop(env_id: str, *, seed: int | None = 42) -> None:
    _log(
        f"env_show: {env_id!r}  MUJOCO_GL={os.environ.get('MUJOCO_GL', '')!r}  "
    ...
    root = tk.Tk()
    root.title(f"Semicircle v6 ({env_id})")
    root.geometry("480x120")
    tk.Label(
        root,
        text="W/S/A/D drive R to restart Q/ESC to exit; automatically restart after hitting a wall/timeout",
    ).pack(pady=20)
    root.focus_force()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Semicircle v6 manual preview")
    p.add_argument(
        "--robot",
        type=str,
        default="Point",
