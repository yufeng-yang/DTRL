"""Method 1 (on-policy E1): PPO.

Fixed logic: Actor is obs→128→128→action_net (consistent with ``pi=[128,128]`` of ``net_arch``).
Loading from SAC weights below **First two paragraphs** Linear: ``latent_pi[0]`` (obs→128), ``latent_pi[2]`` (128→128)
Go to PPO ``policy_net[0]``, ``policy_net[2]`` and freeze; only train 128→actions with the entire Critic (obs→128→V).

Run: python onpolicy_e1_train.py"""
...
def transfer_sac_actor_first_two_linear_frozen(sac_path: Path, model: PPO) -> None:
    """SAC ``actor.latent_pi``: Linear(0)+ReLU(1)+Linear(2)+…; aligned with the first two segments of the b1_full_only deep MLP.
    Copy into PPO ``policy_net[0]``, ``policy_net[2]`` and freeze.    """
    sac = SAC.load(str(sac_path))
    src_seq = sac.policy.actor.latent_pi
    dst_seq = model.policy.mlp_extractor.policy_net
    for si, di in ((0, 0), (2, 2)):
        src = src_seq[si]
        dst = dst_seq[di]
...
def main() -> None:
    ...
    run_dir = root / f"ppo_e1_sac128x128_{ts}"
    ...
    transfer_sac_actor_first_two_linear_frozen(SAC_BEST, model)
    if VERBOSE >= 1:
        print(
            f"[E1] Loaded and frozen policy_net[0,2] from SAC (obs-128-128): {SAC_BEST}\n"
            "[E1] Training action_net and Critic; Actor/Critic has no shared weights"
        )
    ...
    final_path = weights_dir / "ppo_e1_final"
