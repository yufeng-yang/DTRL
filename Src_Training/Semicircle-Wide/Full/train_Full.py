"""Training SAC (Stable-Baselines3) on Semicircle **v6 wide track**.

Default environment ``SafetyPointSemicircle0-v6``; log and checkpoint writing
``<this script directory>/runs/full_sac_<timestamp>/``.

Run::

    cd /home/yufeng.yang/codespace/Prepare_to_transfer/src_training/Semicircle-Wide/Full
    python train_sac_full.py"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train SAC on Safety*Semicircle0-v6 wide track (default Point) with SB3."
    )
    parser.add_argument(
        "--env-id",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Run the root directory; full_sac_<timestamp>/(checkpoints, tb, best_model, final_model) will be created under it."
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="SAC train frequency in env steps. Larger values can improve throughput.",
    )
    parser.add_argument(
        "--gradient-steps",
        type=int,
...
def main():
    ...
    run_dir = os.path.join(args.runs_root, f"Full_{timestamp}")
    ...
    model.learn(
        ...
        callback=callbacks,
        tb_log_name="sac_semicircle",
        progress_bar=args.progress_bar,
    )

    final_model_path = os.path.join(final_model_dir, "sac_semicircle_final")
