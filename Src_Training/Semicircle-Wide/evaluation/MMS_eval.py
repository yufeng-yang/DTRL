"""Semicircle v6 MMS dynamic port selection evaluation (same rules as dtrl_off_policy_eval).

e1: SWI SAC (obs→256) | e2: MMS PPO (obs→256→128→128) | e3: full SAC (deep MLP)
Every 10 step deadline∈[0.08,0.15] ms, press buffer p97 to select the deepest exit; miss→previous step action.
Preheating 1000 steps + filling buffer 1000 steps + official 100 seeds.

Run: python mms_eval.py"""
...
PolicyKind = Literal["sac", "ppo"]

# (label, weight path, type, depth)
_EXITS: list[tuple[str, Path, PolicyKind, int]] = [
    (
        "e1",
        Path(
            "/home/yufeng.yang/codespace/Prepare_to_transfer/src_training/Semicircle-Wide/SWI/runs/"
            "SWI_20260517_130543/checkpoints/SWI_600000_steps.zip"
        ),
        "sac",
        1,
    ),
    (
        "e2",
        Path(
            "/home/yufeng.yang/codespace/Prepare_to_transfer/src_training/Semicircle-Wide/MMS/runs/"
            "MMS_20260517_141104/best_model/MMS_best_model.zip"
        ),
        "ppo",
        2,
    ),
    (
        "e3",
        Path(
            "/home/yufeng.yang/codespace/Prepare_to_transfer/src_training/Semicircle-Wide/Full/runs/"
            "Full_20260517_125954/best_model/Full_best_model.zip"
        ),
        "sac",
        3,
    ),
...
def _timed_infer(
    ...
    with torch.inference_mode():
        ...
        out = exit_actor.actor(x).detach().cpu().numpy().reshape(-1)
        action = _to_env_action(out, action_space, exit_actor.kind)
...
def main() -> None:
    ...
    arch = {
        "e1": "SWI SAC obs→256→out (tanh+unscale)",
        "e2": "MMS PPO obs→256→128→128→out (Gaussian mean)",
        "e3": "full SAC obs→256→128→128→64→64→out (tanh+unscale)",
    }
    ...
    for tag, path, kind, depth in _EXITS:
        ...
        print(f"  [{tag}] {path.name} | {arch[tag]}")
    ...
    print(
        ...
        f"success: reaches the end point and return>{SUCCESS_RETURN_THRESH}"
    )
    print(
