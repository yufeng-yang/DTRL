"""Semicircle v6 cross-exit action inconsistency (pairwise normalized cross-exit action inconsistency).

For MMS/DTRL_ON/DTRL_OFF:
  1. Each exit rolls out N_EPISODES independently and collects S^(i)
  2. S_eval = ⋃_i S^(i) (obs rounding and deduplication)
  3. Use the same three-step reasoning as obs and find the inconsistency mean according to the formula
...
DTRL_ON_PATHS: list[tuple[str, Path]] = [
    (
        "e1",
        Path(
            "/home/yufeng.yang/codespace/Prepare_to_transfer/Src_Training/Semicircle-Wide/DTRL-On/runs/"
            "DTRL-On_e1_20260517_161241/best_model/DTRL-On_e1_best_model.zip"
        ),
    ),
    (
        "e2",
        Path(
            "/home/yufeng.yang/codespace/Prepare_to_transfer/Src_Training/Semicircle-Wide/DTRL-On/runs/"
            "DTRL-On_e2_20260517_190831/best_model/DTRL-On_e2_best_model.zip"
        ),
    ),
    (
        "ef",
        Path(
            "/home/yufeng.yang/codespace/Prepare_to_transfer/Src_Training/Semicircle-Wide/DTRL-On/runs/"
            "DTRL-On_full_20260517_145457/best_model/DTRL-On_full_best_model.zip"
        ),
    ),
]
...
def main() -> None:
    ...
    print(
        f"|S_eval| is the union of three rollout states (remove duplication)"
    )
    print(
    ...
    print(f"\n{'=' * 70}")
    print("Summary (in ascending order by inconsistency mean, the lower the more consistent):")
