"""Semicircle v6 fixed SWI (shallow SAC) formal evaluation (same time constraints as full_eval / dtrl_off_policy_eval).

Strategy: b2_swi obs→256→out
...
Success: Reach the end point at the end of the game and return>9

Run: python swi_eval.py"""
...
class SacSwiActor(nn.Module):
    """SWI shallow SAC: latent_pi(256) → mu → tanh."""
...
def _timed_infer(
    actor: SacSwiActor, obs: np.ndarray, action_space: spaces.Box
...
def _warmup(actor: SacSwiActor, env_id: str, action_space: spaces.Box, seed: int) -> None:
...
def _run_episode(
    actor: SacSwiActor, env_id: str, action_space: spaces.Box, seed: int
) -> EpisodeStats:
    _setup_cpu(quiet=True)
    env = _make_env(env_id)
...
def main() -> None:
    ...
    print("Strategy: SWI Shallow SAC (b2_swi, obs→256→out)")
    ...
    print(f"\n{'=' * 70}")
    print(f"comprehensive ({N_SEEDS} seeds) — SWI Shallow SAC:")
