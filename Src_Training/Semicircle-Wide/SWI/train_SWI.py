"""Train SWI SAC on Semicircle **v6 wide track** (Shallow 256, Stable-Baselines3).

Strategy/value network: obs → 256 → output (pi / qf independent MLP).
Default environment ``SafetyPointSemicircle0-v6``; log and checkpoint writing
``<this script directory>/runs/swi_sac_<timestamp>/`` (i.e. ``b2_swi/runs/...``).

Run::

    cd /home/yufeng.yang/codespace/Prepare_to_transfer/src_training/Semicircle-Wide/SWI
    python train_swi_sac.py"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train SWI SAC (256 MLP) on Safety*Semicircle0-v6 wide track with SB3."
    )
    parser.add_argument(
        "--env-id",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Run the root directory; swi_sac_<timestamp>/(checkpoints, tb, best_model, final_model) will be created under it."
    ...
    parser.add_argument("--device", type=str, default="auto", help="SAC device: auto/cpu/cuda/cuda:0 ...")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="SAC train frequency in env steps.",
    )
    parser.add_argument(
        "--gradient-steps",
        type=int,
        default=1,
        help="SAC gradient steps after each train trigger.",
    )
    parser.add_argument(
        "--progress-bar",
        action="store_true",
...
def main():
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        save_path=ckpt_dir,
        name_prefix="swi_sac_semicircle",
    ...
    print(f"[INFO] net_arch pi/qf=[256] (SWI)")
    ...
    model.learn(
        ...
        callback=callbacks,
        tb_log_name="swi_sac_semicircle",
        progress_bar=args.progress_bar,
    )

    final_model_path = os.path.join(final_model_dir, "swi_sac_semicircle_final")
