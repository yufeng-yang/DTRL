"""Semicircle **v6 wide track** three-port combined SAC (e1/e2/efull).

- e1: obs → 256 → out
- e2: obs → 256 → 128 → 128 → out
- efull (exit3): obs → 256 → 128 → 128 → 64 → 64 → out
- Before ``--warmup-steps`` (default **800k**) only efull backpropagation; after weighted union e1/e2/efull
- Total training ``--total-timesteps`` default **1M**
- 16 channels of parallel sampling; write ``eval/*`` for three evals (seed 42–46) every 10k in the whole process; write ``eval_fine/*`` every 100 steps for 800k–830k

Run::

    cd /home/yufeng.yang/codespace/Prepare_to_transfer/src_training/Semicircle-Wide/DTRL-Off
    conda activate sb3sg && python joint_train.py"""
...
_SEMICIRCLE_BOARD = Path(__file__).resolve().parents[1]
if str(_SEMICIRCLE_BOARD) not in sys.path:
    sys.path.insert(0, str(_SEMICIRCLE_BOARD))
...
def write_eval_returns_tensorboard(
    ...
    *,
    ...
    """Three-port mean return written to TensorBoard (``eval/`` or ``eval_fine/``)."""
    writer.add_scalar(f"{prefix}/mean_reward_e1", r1, global_step)
    writer.add_scalar(f"{prefix}/mean_reward_e2", r2, global_step)
    writer.add_scalar(f"{prefix}/mean_reward_efull", r3, global_step)
    writer.add_scalar(f"{prefix}/weighted_mean_return", weighted, global_step)


def run_three_exit_eval(
    args: argparse.Namespace,
    actor: ActorEENN3Semicircle,
    device: torch.device,
    action_space: spaces.Box,
) -> tuple[float, float, float, float]:
    return eval_three_exit_weighted_return(
        args.env_id,
        actor,
        device,
        action_space,
...
def log_train_episode_returns(
    writer: SummaryWriter,
    infos,
    global_step: int,
) -> None:
    """Write at the end of any episode in parallel env (frequency ≈ once per round, non-fixed step interval)."""
    for info in infos:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Semicircle v6 three-port joint SAC (e1/e2/efull)")
    ...
    p.add_argument(
        ...
        type=int,
        default=800_000,
        help="Previously, actor only efull(exit3) backpass; combined e1/e2 loss after reaching",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=10_000,
        help="At every env step in the whole process, eval is executed three times and eval/* is written (default 10k).",
    )
    p.add_argument(
        "--eval-episodes",
        type=int,
        default=5,
        help="The number of rounds for each eval; the default seed is 42..46, a total of 5 rounds.",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=800_000,
        help="intensive eval starting env step (inclusive).",
    )
    p.add_argument(
        "--eval-fine-end",
        type=int,
        default=830_000,
        help="intensive eval ends env step (inclusive).",
    )
    p.add_argument(
        "--eval-fine-freq",
        type=int,
        default=100,
        help="[eval_fine_start, eval_fine_end] Execute eval three times every few steps and write eval_fine/*.",
    )
...
def train(args: argparse.Namespace) -> None:
    ...
    next_fine_eval_at = args.eval_fine_start
    next_save_at = args.save_freq
    eval_seed_end = args.eval_start_seed + args.eval_episodes - 1

    print(f"[INFO] env_id={args.env_id}")
    print(f"[INFO] total_timesteps={args.total_timesteps}")
    print(f"[INFO] warmup_steps={args.warmup_steps} (efull only, then joint e1/e2/efull)")
    print(f"[INFO] joint_phase_steps={max(0, args.total_timesteps - args.warmup_steps)}")
    print(f"[INFO] n_envs={args.n_envs}, device={device}")
    print(f"[INFO] save_freq={args.save_freq}")
    print(
        f"[INFO] TB eval/*: each {args.eval_freq} env steps, three mouths,"
        f"seeds {args.eval_start_seed}..{eval_seed_end}（{args.eval_episodes} Bureau)"
    )
    print(
        f"[INFO] TB eval_fine/*: step ∈ [{args.eval_fine_start}, {args.eval_fine_end}] "
        f"Every {args.eval_fine_freq} steps, same as above for three mouths"
    )
    print(f"[INFO] TB loss/*: per {args.log_interval} env steps (after training starts)")
    print(
        f"[INFO] best_model updated by eval/weighted_mean_return (only {args.eval_freq} regular eval)"
    )
    ...
    while global_step < args.total_timesteps:
        ...
        log_train_episode_returns(writer, infos, global_step)
        ...
        while global_step >= next_eval_at:
            eval_step = next_eval_at
            next_eval_at += args.eval_freq
            r1, r2, r3, weighted = run_three_exit_eval(
                args, actor, device, vec_env.action_space
            )
            write_eval_returns_tensorboard(writer, eval_step, r1, r2, r3, weighted, prefix="eval")
            print(
        ...
        while (
            next_fine_eval_at <= args.eval_fine_end
            and global_step >= next_fine_eval_at
        ):
            fine_step = next_fine_eval_at
            next_fine_eval_at += args.eval_fine_freq
            r1, r2, r3, weighted = run_three_exit_eval(
                args, actor, device, vec_env.action_space
            )
            write_eval_returns_tensorboard(
                writer, fine_step, r1, r2, r3, weighted, prefix="eval_fine"
            )
            print(
                f"[EvalFine] step={fine_step} "
                f"e1={r1:.4f} e2={r2:.4f} efull={r3:.4f} weighted={weighted:.4f}"
            )
