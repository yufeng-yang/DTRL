"""Semicircle track v6: Same geometry and hazard density as v2/v5, lane half width is 2 times that of v5."""

import numpy as np

# ---------- Semicircle parameters (aligned with v2, only LANE_HALF_WIDTH doubled) ----------
RADIUS = 2.0
GOAL_SIZE = 0.25
GOAL_X = RADIUS
GOAL_Y = 0.0
...
# v5 (v2) is 0.35; v6 lane full width doubled => half width × 2
LANE_HALF_WIDTH_V5 = 0.35
LANE_HALF_WIDTH = LANE_HALF_WIDTH_V5 * 2.0
...
def goal_xy_v5_aligned() -> tuple[float, float]:
    """The same world coordinates as v5: at arc length 5/8, according to the v5 inner ring welding formula."""
    s_arc = 5.0 / 8.0
    theta = np.pi * (1.0 - s_arc)
    center_r = (RADIUS - LANE_HALF_WIDTH_V5) + GOAL_SIZE
    return float(center_r * np.cos(theta)), float(center_r * np.sin(theta))
