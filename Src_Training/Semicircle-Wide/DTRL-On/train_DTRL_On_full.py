"""Training PPO full MLP (Stable-Baselines3) on Semicircle **v6 wide track**.

Strategy/value network: obs → 256 → 128 → 128 → 64 → 64 → output (pi/vf each independent MLP).
Default 16-way parallel environment; total number of steps: 1.5 million; parallel eval of 10 rounds (different seeds) every 50,000 steps to save best_model.
Logs and checkpoints are written to ``<this script directory>/runs/full_ppo_<timestamp>/`` by default.

Run::

    cd /home/yufeng.yang/codespace/Prepare_to_transfer/src_training/Semicircle-Wide/DTRL-On
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train PPO (full MLP) on Safety*Semicircle0-v6 wide track with SB3."
    )
    parser.add_argument(
        "--env-id",
        type=str,
        default="SafetyPointSemicircle0-v6",
        help="Must be registered Semicircle v6.",
    )
