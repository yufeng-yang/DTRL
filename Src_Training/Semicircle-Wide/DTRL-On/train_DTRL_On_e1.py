"""Semicircle v6 PPO Stage-E1: Freeze the obs of full PPO→256, and only retrain the action header.

Loading ``policy_net[0]`` from ``train_ppo_full`` weights (``pi=[256,128,128,64,64]``) (obs→256)
And freeze; this script ``pi=[256]`` (obs→256→out), only trains ``action_net`` (256→action distribution)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="PPO E1 stage on Semicircle v6 (frozen obs→256 trunk).")
