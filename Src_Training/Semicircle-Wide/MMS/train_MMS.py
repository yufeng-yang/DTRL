"""Training PPO (MMS, Stable-Baselines3) on Semicircle **v6 wide track**.

Policy/value network: obs → 256 → 128 → 128 → output (pi / vf independent MLP, no shared feature extractor).
Default environment ``SafetyPointSemicircle0-v6``; default ``device=cpu``; eval 1 round every 50,000 steps (the indicator is written to TensorBoard, and press return to save best);
Log and checkpoint writing
``<this script directory>/runs/mms_ppo_<timestamp>/`` (i.e. ``b3_mms/runs/...``).

Run::

    cd /home/yufeng.yang/codespace/Prepare_to_transfer/src_training/Semicircle-Wide/MMS
    python train_mms_ppo.py"""
...
# Semicircle_env is located under Semicircle-Wide/ (the upper-level directory of this script)
...
class SafetyToGymnasiumWrapper(gym.Wrapper):
    """Safety-Gymnasium → Gymnasium step API。"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train PPO (MMS [256,128,128]) on Safety*Semicircle0-v6 wide track with SB3."
    )
    parser.add_argument(
        "--env-id",
        type=str,
        default="SafetyPointSemicircle0-v6",
        help="Must be a registered Semicircle v6, for example SafetyCarSemicircle0-v6.",
    )
    ...
    parser.add_argument(
        ...
        help="Run the root directory; mms_ppo_<timestamp>/(checkpoints, tb, best_model, final_model) will be created under it."
    ...
    parser.add_argument("--device", type=str, default="cpu", help="PPO device, default cpu.")
...
def main():
    ...
    run_dir = os.path.join(args.runs_root, f"MMS_{timestamp}")
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        save_path=ckpt_dir,
        name_prefix="mms_ppo_semicircle",
    ...
    print(f"[INFO] net_arch pi/vf=[256, 128, 128] (obs-256-128-128-out, no shared features)")
    ...
    print(f"[INFO] eval_tensorboard_scalars=eval/mean_reward,eval/mean_ep_length,eval/std_reward")
    ...
    model.learn(
        ...
        callback=callbacks,
        tb_log_name="mms_ppo_semicircle",
        progress_bar=args.progress_bar,
    )

    final_model_path = os.path.join(final_model_dir, "mms_ppo_semicircle_final")
